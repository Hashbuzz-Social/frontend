...
import { delay } from '../../../comman/helpers';
...
export const CreateTwitterPage = () => {
  ...
  const connectHashpack = async () => {
    try {
      if (isDeviceIsSm) {
        toast.warning(
          'Please connect with HashPack extension on your desktop to make a payment'
        );
        return alert(
          'Please connect with HashPack extension on your desktop to make a payment'
        );
  ...
  const getCampaignList = async () => {
    try {
      ...
      if (response && response.length > 0) {
        ...
        let pendingResult = response.find(
          data => data.card_status === 'Pending'
        );
        let runningResult = response.find(
          data => data.card_status === 'Running'
        );
        ...
        if (pendingResult)
          store.updateState(d => ({ ...d, currentStatus: 'Pending Approval' }));
        if (runningResult)
          store.updateState(d => ({ ...d, currentStatus: 'Running' }));
      }
    } catch (err) {
  ...
  return (
    <ContainerStyled align='center' justify='space-between'>
      {store &&
      ['Ashh20x', 'omprakashMahua'].includes(store?.user?.username) ? (
        <LinkContainer>
          <Link to='/admin'>
            <p>Admin Panel</p>
          </Link>
      ...
      <CardSection>
        <StatusCard
          ...
          buttonTag={[
            `${!store?.currentUser?.hedera_wallet_id ? 'Connect' : ''}`,
          ]}
        ...
        <StatusCard
          ...
          content={
            (store?.available_budget / Math.pow(10, 8)).toFixed(3) + ' ℏ'
          }
        ...
        <StatusCard
          ...
          content={
            store?.user?.business_twitter_handle
              ? '@' + store?.user?.business_twitter_handle
              : ''
          }
          buttonTag={[
            !store?.user?.business_twitter_handle
              ? 'Connect brand handle'
              : 'Reconnect',
          ]}
        ...
        <StatusCard
          ...
          content={
            store?.user?.personal_twitter_handle
              ? '@' + store?.user?.personal_twitter_handle
              : ''
          }
          buttonTag={['ReConnect']}
        ...
        />
        {tableData.length > 0 ? (
          <StatusCard title={'Status'} content={store?.currentStatus} />
        ) : null}
      </CardSection>
          ...
          <TableBody>
            {tableData.map((item, index) => (
              <TableRow>
                ...
                <CustomTableBodyCell>
                  {(item.campaign_budget / Math.pow(10, 8)).toFixed(4)}
                </CustomTableBodyCell>
                <CustomTableBodyCell>
                  {(item.amount_spent / Math.pow(10, 8)).toFixed(4)}
                </CustomTableBodyCell>
                <CustomTableBodyCell>
                  {(item.amount_claimed / Math.pow(10, 8)).toFixed(4)}
                </CustomTableBodyCell>
                <CustomTableBodyCell>
      ...
      <StatusSection>
        During the beta phase, there is a limitation of running a single
        campaign concurrently. Each campaign will conclude automatically 24
        hours after its initiation, unless you choose to end it earlier. We
        anticipate relaxing these constraints gradually. Additionally, your
        recharged balance is available for unlimited use across various
        campaigns.
      </StatusSection>
      <PrimaryButton
