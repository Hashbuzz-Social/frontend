...
const TopUpModal = ({ open, setOpen, isTopUp }) => {
  ...
  const handleBudget = event => {
    ...
    if (
      Math.round(event.target.value * Math.pow(10, 8)) <= state.available_budget
    ) {
      ...
      setBudgetMessage(
        `You have exceeded the total budget of ${state.available_budget / Math.pow(10, 8)} ℏ`
      );
  ...
  const submitPay = async e => {
    if (!pairingData) {
      ...
      return toast.warning(
        'Connect Your wallet first then try to top-up again.'
      );
    }
    ...
    try {
      ...
      const transaction = await topUpAccount(
        { ...amounts },
        state?.user?.hedera_wallet_id
      );
    ...
    >
      <BoxCont>
        <Typography theme={theme}>
          {isTopUp ? 'TopUp' : 'Reimburse'} account
        </Typography>
        <Row>
          <>
          ...
          <>
            {isTopUp ? (
              <>
                <Label>
                  + {(hbarTotinuHbar(amount).fee / Math.pow(10, 8)).toFixed(3)}
                </Label>
                <Label>=</Label>
                <Label>
                  {(hbarTotinuHbar(amount).total / Math.pow(10, 8)).toFixed(3)}
                </Label>
              </>
            ) : (
              <Label></Label>
        ...
        <div>
          <ErrorTextWrap style={{ marginTop: '-20px' }}>
            {budgetMessage}
          </ErrorTextWrap>
        </div>
        {isTopUp ? (
          <>
            ...
            <CustomParagraph>
              Note3: hashbuzz applies 10% charge fee on top of the specified
              amount
            </CustomParagraph>
          </>
        ) : (
      ...
      <ButtonWrapPrimary>
        ...
        {isTopUp ? (
          ...
          <PrimaryButton
            disabled={buttonDisabled || !amount || amount < 1}
            text={'Reimburse'}
